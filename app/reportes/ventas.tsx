...
const categoriaDesdeAfectacion = (c: DNITVenta): string => {
...
export default function ReportesVentas() {
  ...
  const [ventas, setVentas] = useState<DNITVenta[]>([]);
  ...
  useEffect(() => {
    ...
    const ventasRaw = ((data as any).ventas || []) as DNITVenta[];
    const depVentas = ventasRaw.filter((v) => !v.noImputar);
    setVentas(depVentas);

    const periodos = Array.from(new Set(depVentas.map(periodoKey)))
      .filter((p) => p !== "00/0000")
      .sort(sortPeriodos);
  ...
  const { kpiMensual, categoriasAfectacion } = useMemo(() => {
    ...
    const ingresosNetos = Math.max(0, ingresos);
  ...
  /* ===== Top clientes MENSUAL ===== */
  const topClientesMensual = useMemo(() => {
    ...
    filtradas.forEach((c) => {
      const total = montoTotal(c);
      const abs = Math.abs(total);
      if (isNCVenta(c) || total < 0) return;

      const clave = (c.clienteNombre || c.clienteRuc || "Sin cliente").trim();
      map.set(clave, (map.get(clave) || 0) + abs);
    });
  ...
  const totalClientesMontoMensual = useMemo(
    () => topClientesMensual.reduce((s, p) => s + p.monto, 0),
    [topClientesMensual],
  );
  ...
  const anual = useMemo(() => {
    ...
    const byMonth = Array.from({ length: 12 }, () => ({ ing: 0 }));
    ...
    ventas.forEach((c) => {
      ...
      if (!isNCVenta(c) && total > 0) byMonth[m - 1].ing += abs;
    });

    const fullData = byMonth.map((m, i) => ({
      label: monthNames[i],
      value: Math.round((Math.max(0, m.ing) / 1_000_000) * 10) / 10,
    }));
    ...
    const ingresosYear = byMonth.reduce((s, m) => s + Math.max(0, m.ing), 0);
    ...
    return { data: trimmedData, ingresosYear, comprobantesYear, maxValue };
  }, [ventas, year]);
  ...
  /* ===== Top clientes ANUAL ===== */
  const topClientesAnual = useMemo(() => {
  ...
  const totalClientesMontoAnual = useMemo(
    () => topClientesAnual.reduce((s, p) => s + p.monto, 0),
    [topClientesAnual],
  );
    ...
    return (
      <SafeAreaView style={tw`flex-1 bg-gray-100`} edges={["bottom"]}>
        <View style={tw`flex-1 justify-center items-center`}>
          ...
          <Text style={tw`mt-2 text-gray-600`}>Cargando ventas…</Text>
        </View>
      </SafeAreaView>
    );
  }
        ...
        <ScrollView contentContainerStyle={tw`px-4 pb-6`}>
          <View style={[tw`bg-white rounded-2xl p-4`, SHADOW]}>
            ...
            <Text style={tw`text-sm text-gray-600`}>
              Aún no encontramos ventas cargadas en tu DNIT. Volvé a{" "}
              <Text
      ...
      <ScrollView contentContainerStyle={tw`px-4 pb-6`}>
        <View style={[tw`bg-white rounded-2xl p-4 mb-3`, SHADOW]}>
          <Text style={tw`text-base text-gray-800`}>
            <Text style={tw`font-semibold`}>Tus VENTAS</Text> declaradas en tu
            DNIT. Acá ves tu facturación y distribución de ingresos. ¿Dudas?{" "}
            <Text
              style={tw`text-blue-600 underline`}
        ...
        {mode === "mensual" && (
          <>
            ...
            <View style={[tw`bg-white rounded-2xl p-4 mt-4`, SHADOW]}>
              <Text style={tw`text-base font-bold mb-3`}>
                Top clientes • {month ? monthNames[month - 1] : ""} {year ?? ""}
              </Text>

              {!year || !month ? (
                <Text style={tw`text-xs text-gray-500`}>
                  Elegí año y mes para ver los principales clientes.
                </Text>
              ) : topClientesMensual.length === 0 ? (
                <Text style={tw`text-xs text-gray-500`}>
                ...
                <View style={tw`max-h-64`}>
                  <ScrollView nestedScrollEnabled>
                    {topClientesMensual.map((p, i) => {
                      const pct = totalClientesMontoMensual
                        ? Math.round(
                            (p.monto * 100) / totalClientesMontoMensual,
                          )
                        : 0;
          ...
          <>
            ...
            {year && (
              <View style={tw`flex-row flex-wrap -mx-1 mt-3 mb-1`}>
                <KpiCard
                  title="Ingresos (Ventas año)"
                  value={anual.ingresosYear}
                  style="bg-green-100"
                />
                <KpiCard
            ...
            <View style={[tw`bg-white rounded-2xl p-4`, SHADOW]}>
              ...
              <Text style={tw`text-xs text-gray-500 mt-2`}>
                Total ingresos (ventas) del año:{" "}
                <Text style={tw`font-semibold`}>
                  {peso(Math.round(anual.ingresosYear))}
                </Text>
              </Text>
            ...
            <View style={[tw`bg-white rounded-2xl p-4 mt-4`, SHADOW]}>
              ...
              {!year ? (
                <Text style={tw`text-xs text-gray-500`}>
                  Elegí un año para ver los clientes principales.
                </Text>
              ) : topClientesAnual.length === 0 ? (
                <Text style={tw`text-xs text-gray-500`}>
                ...
                <View style={tw`max-h-64`}>
                  <ScrollView nestedScrollEnabled>
                    {topClientesAnual.map((p, i) => {
                      const pct = totalClientesMontoAnual
                        ? Math.round((p.monto * 100) / totalClientesMontoAnual)
                        : 0;
